@extends('master')

@section('content')

@if($message = Session :: get('success'))
<div class="alert alert-success">

    {{$message}}
    
</div>

@endif

<div class="card">
    <div class="card-header bg-success">
        <div class="row">
            <div class="col col-md-6"><h4>Student's Data</h4></div>
            <div class="col col-md-6">
                <a href="{{route('students.create')}}" class="btn btn-primary btn-lg float-end">Add</a>
            </div>
        </div>

    </div>
    <div class="card-body">
        <table class="table table-bordered">
            <tr>
                <th>Student_Image{{$i}}</th>
                <th>Student_Name</th>
                <th>Student_Rollno</th>
                <th>Student_Marks</th>
                <th>Student_Address</th>
                <th>Action</th>
            </tr>
            @if(count($data) >0)
                 @foreach($data as $row)

            <tr>
                <td><img src="{{asset('images/'.$row->student_image)}}" class="rounded-circle img-sm" width="50"/></td>
                <td>{{$row->student_name}}</td>
                <td>{{$row->student_rollno}}</td>
                <td>{{$row->student_marks}}</td>
                <td>{{$row->student_address}}</td>
                <td>
                <form method="post" action=" {{ route('students.destroy' , $row->id )}}">
                        @csrf
                        @method('DELETE')
                        <a href="{{route('students.show', $row->id)}}" class="btn btn-primary btn-sm">View></a>

                        <a href="{{route('students.edit', $row->id)}}" class="btn btn-warning btn-sm">Edit></a>

                        <input type="submit" class="btn btn-danger btn-sm" value="Delete"/>
                 

                    </form>
                </td>
                
            </tr>
            @endforeach
            @else
            <tr>
                <td colspan="5" class="text-center">No Data Found</td>
            </tr>
            @endif
        </table>
        {!! $data->links() !!}
    </div>
</div>
@endsection
